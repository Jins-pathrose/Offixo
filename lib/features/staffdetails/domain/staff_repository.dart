...
class Staffrepository {
  ...
  Future<Map<String, String>> _getHeaders() async {
    ...
    print(
      '🔑 Using token: ${cleanToken.substring(0, cleanToken.length > 15 ? 15 : cleanToken.length)}...',
    );
  ...
  Future<LeaveAnalyticsResponse> getLeaveAnalytics(
    ...
    final url =
        '$baseUrl/api/member/leave-analytics/?member_id=$memberId&year=$year&month=$month';
    print('🌐 [$apiName] GET: $url');
  ...
  Future<LeaveBalanceResponse> getLeaveBalance(int memberId, int year) async {
    final apiName = 'getLeaveBalance';
    final url =
        '$baseUrl/api/member/leave-balance-per-person/?member_id=$memberId&year=$year';
    print('🌐 [$apiName] GET: $url');

    final headers = await _getHeaders();

    final response = await http.get(Uri.parse(url), headers: headers);
    print('response Leave Balance : ${response.body}');
    final data = await _handleResponse(response, apiName);
  ...
  Future<PayrollListResponse> getPayrollList(int memberId) async {
    ...
    if (response.statusCode == 200) {
      ...
      print('❌ [$apiName] Response body: ${response.body}');
      print(
        '❌ [$apiName] This endpoint may not be implemented or has a bug on the server.',
      );
      ...
      print('⚠️ [$apiName] Response: ${response.body}');
      throw Exception(
        'Failed to load payroll list: ${response.statusCode}\n${response.body}',
      );
    }
  }

  Future<PayslipResponse> getPayslip(int memberId, int month, int year) async {
    ...
    if (response.statusCode == 200) {
      ...
      print('⚠️ [$apiName] Response: ${response.body}');
      throw Exception(
        'Failed to get payslip: ${response.statusCode}\n${response.body}',
      );
    }
  }

  Future<Payslip?> getPayslipPreview(int memberId, int month, int year) async {
    final apiName = 'getPayslipPreview';
    final url =
        '$baseUrl/api/salary/payslips/preview/?month=$month&year=$year&member_id=$memberId';
    print('🌐 [$apiName] GET: $url');
  ...
  Future<void> generatePayslip(int memberId, int month, int year) async {
    ...
    final response = await http.post(
      Uri.parse(url),
      headers: headers,
      body: json.encode({'month': month, 'year': year, 'member_id': memberId}),
    );
  ...
  Future<void> downloadPayslip(int payslipId) async {
    ...
    if (response.statusCode == 500) {
      ...
      print('⚠️ [$apiName] Response: ${response.body}');
      throw Exception(
        'Failed to download payslip: ${response.statusCode}\n${response.body}',
      );
    }
  }
  ...
  Future<void> updateStaffDetails(
    ...
    if (response.statusCode != 200 && response.statusCode != 201) {
      ...
      print('⚠️ [$apiName] Response: ${response.body}');
      throw Exception(
        'Failed to update staff details: ${response.statusCode}\n${response.body}',
      );
    }
  }
